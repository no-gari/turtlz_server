...
class Catalog(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.PROTECT, verbose_name='작성')
...
class Magazines(models.Model):
    ...
    like_users = models.ManyToManyField(User, related_name='like_magazines', verbose_name='이 매거진을 좋아한 사람들', null=True, blank=True)
    scrapped_users = models.ManyToManyField(User, related_name='scrapped_magazines', verbose_name='이 매거진을 스크랩한 사람들', null=True, blank=True)
    user = models.ForeignKey(User, on_delete=models.PROTECT, verbose_name='작성')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='생성일')
    updated_at = models.DateTimeField(auto_now=True, verbose_name='수정일')
    ...
    order = models.IntegerField(default=1, verbose_name='메인배너 순서', help_text='숫자가 적을수록 먼저 나옵니다.')
    magazine_url = models.URLField(verbose_name='우피 url', null=True, blank=True)
